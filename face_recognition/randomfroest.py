...
rfc = RandomForestClassifier(n_estimators=300,max_depth=500)
rfc = rfc.fit(x_train,y_train)
print('测试集acc=',rfc.score(x_test,y_test))
...
for i in range(y_train.shape[0]):
    ...
    if (y_train[i] == y_train_predict[i]):
        right = right + 1
acc = right / y_train.shape[0]
print("训练集准确率为：", acc)
if(peoplenum==2):

    y_pred_gbc = rfc.predict_proba(x_test)[:, 1]  ###这玩意就是预测概率的
